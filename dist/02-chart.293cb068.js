...
function ready(datapoints) {
  ...
  svg.selectAll('path').data(nested).enter().append('path').attr('class', 'path-line').attr('d', function (d) {
    return line(d.values);
  }).attr('stroke', function (d) {
    return colorScale(d.key);
  }).attr('stroke-width', 2).attr('fill', 'none').attr('id', function (d) {
    return d.key.toLowerCase().replace(/[^a-z]/g, '');
  });
  svg.selectAll('circle').data(nested).enter().append('circle').attr('class', 'circle-small').attr('fill', function (d) {
    return colorScale(d.key);
  ...
  svg.selectAll('text-label').data(nested).enter().append('text').attr('class', 'text-label').attr('y', function (d) {
    return yPositionScale(d.values[0].price);
  }).attr('x', function (d) {
    return xPositionScale(d.values[0].datetime);
  }).text(function (d) {
    return d.key;
  }).attr('dx', 6).attr('dy', 4).attr('font-size', '12').attr('id', function (d) {
    return d.key.toLowerCase().replace(/[^a-z]/g, '');
  });
  svg.append('text').attr('font-size', '24').attr('text-anchor', 'middle').text('U.S. housing prices fall in winter').attr('x', width / 2).attr('y', -40).attr('dx', 40);
  var rectWidth = xPositionScale(parseTime('February-17')) - xPositionScale(parseTime('November-16'));
  svg.append('rect').attr('x', xPositionScale(parseTime('December-16'))).attr('y', 0).attr('width', rectWidth).attr('height', height).attr('fill', '#C2DFFF').lower(); // This is going to select what to raise in the grapth
  ...
  d3.select('#us-step').on('stepin', function () {
    ...
    svg.selectAll('text#us').attr('fill', 'red').raise();
    svg.selectAll('path#us').attr('stroke', 'red').raise();
    svg.selectAll('circle#us').attr('fill', 'red').raise();
  });
  d3.select('#region-step').on('stepin', function () {
    svg.selectAll('path').attr('stroke', 'lightgrey');
    svg.selectAll('circle').attr('fill', 'lightgrey');
    svg.selectAll('rect').attr('fill', 'white');
    svg.selectAll('text#us').attr('fill', 'red').attr('font-weight', 700).raise();
    svg.selectAll('path#us').attr('stroke', 'red').raise();
    svg.selectAll('circle#us').attr('fill', 'red').raise();
    svg.selectAll('text#pacific').attr('fill', 'blue').raise();
    svg.selectAll('path#pacific').attr('stroke', 'blue').raise();
    svg.selectAll('circle#pacific').attr('fill', 'blue').raise();
    svg.selectAll('text#mountain').attr('fill', 'blue').raise();
    svg.selectAll('path#mountain').attr('stroke', 'blue').raise();
    svg.selectAll('circle#mountain').attr('fill', 'blue').raise();
    svg.selectAll('text#southatlantic').attr('fill', 'blue').raise();
    svg.selectAll('path#southatlantic').attr('stroke', 'blue').raise();
    svg.selectAll('circle#southatlantic').attr('fill', 'blue').raise();
    svg.selectAll('text#westsouthcentral').attr('fill', 'blue').raise();
    svg.selectAll('path#westsouthcentral').attr('stroke', 'blue').raise();
    svg.selectAll('circle#westsouthcentral').attr('fill', 'blue').raise();
  });
  d3.select('#rect').on('stepin', function () {
    svg.selectAll('path').attr('stroke', 'lightgrey');
    svg.selectAll('circle').attr('fill', 'lightgrey');
    svg.selectAll('rect').attr('fill', '#C2DFFF').attr('opacity');
    svg.selectAll('text#us').attr('fill', 'red').attr('font-weight', 700).raise();
    svg.selectAll('path#us').attr('stroke', 'red').raise();
    svg.selectAll('circle#us').attr('fill', 'red').raise();
    svg.selectAll('text#pacific').attr('fill', 'blue').raise();
  ...
  function render() {
    ...
    yPositionScale.range([newHeight, 0]); // Update things you draw

    svg.selectAll('.circle-small').transition().ease(d3.easeElastic).attr('cy', function (d) {
      return yPositionScale(d.values[0].price);
    }).attr('cx', function (d) {
      return xPositionScale(d.values[0].datetime);
    });
    ...
    svg.selectAll('.text-label').attr('y', function (d) {
      return yPositionScale(d.values[0].price);
    }).attr('x', function (d) {
      return xPositionScale(d.values[0].datetime);
    }).text(function (d) {
    ...
    svg.select('rect').attr('x', xPositionScale(parseTime('December-16'))).attr('y', 0).attr('width', rectWidth).attr('height', height).attr('fill', '#C2DFFF').lower(); // Update axes
